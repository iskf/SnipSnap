...
<p align="center">
  <b>基于原生技术构建的 macOS 高性能截图、长截图拼接、GIF 动图录制、无级贴图、本地离线 OCR 与 Safari 风格原地屏幕翻译工具。</b>
</p>
...
**SnipSnap** 专为追求极致性能与设计质感的 macOS 深度用户、开发者与设计师打造，底层完全采用苹果官方原生技术栈开发（**Swift、AppKit、SwiftUI、ScreenCaptureKit、Apple Vision 与 Apple Translation 框架**）。

拒绝动辄占用数百兆内存的 Electron 封装方案，SnipSnap 具备瞬时启动响应与极低的常驻内存消耗。它将 **Snipaste** 细腻自然的贴图操作习惯，与现代 macOS 苹果人机交互设计规范（HIG）、优雅的柔光质感、智能长截图拼接及 Safari 风格的原地屏幕翻译深度融合。
...
| **发丝级十字准星 · 光学归一化图标 · 智能居中工具条** | **离线 Apple Vision OCR · 视界原地覆盖 · 原文/译文秒级切换** |
...
### 2. 长截图连续自动拼接  *(v1.1.0 新增)*
- **区域连续向下拼接**：框选目标滚动区域（网页、代码、长文档、聊天记录），滚动页面即可自动完成多帧精确重叠计算与无缝缝合。
- **高鲁棒性拼接引擎**：内置自主研发的 `ScrollingStitcher`，采用多级采样条带比对算法与容错阈值机制，无视动态背景干扰，精确锁定重叠像素行。
- **双态自适应 HUD 交互**：
  - **折叠胶囊态**：紧凑展示当前捕获高度与累计帧数，极大减少对视线的遮挡。
  - **展开画卷预览态**：点击或悬浮可实时展开已拼接全景微缩预览，提供直观的高清画面反馈。
- **极致视觉与浅色抗冲突设计**：预览浮层采用双层深色半透明毛玻璃滤镜与柔和微光边缘，并特别针对白色网页等高亮背景优化高对比度边框与专属完成按钮，杜绝文字混淆与视觉冲突。
- **一键快捷唤起**：支持全局快捷键 `Option + F1` 或 `Option + S` 瞬时进入长截图模式。

### 3. GIF 动图录制  *(v1.0.2 新增)*
...
- **自适应动态宽度与平滑动画**：控制条根据当前状态自适应调整宽度与按钮布局，并保持右侧锚定平滑展开。
...
### 6. 矢量标注工具箱与交互体验升级  *(v1.1.0 升级)*
- **极细发丝级十字准星（Hairline Crosshair）**：启动截图时的取景十字准星精细至 1px 半透明线条，中心镂空 16px，实现无遮挡精准定位。
- **482pt 紧凑型主工具条**：重塑布局层级，主工具条瘦身至 482pt；当截图选区较小时，工具条自动居中贴合在选区正下方，杜绝突兀溢出。
- **全图标光学平衡归一化（Optical Normalization）**：对所有 16 个工具及操作图标进行比例与重心重算，使方形、圆形、对角线和文本图标在视觉感受上尺寸完全均一。
- **翡翠绿高亮完成按钮**：主完成按钮升级为醒目翡翠绿背景，操作视觉动线更加清晰流畅。
- **二级文字选项像素级居中**：二级调色板内的文字样式（“纯色”、“描边”）与字号（“14”、“18”、“24”、“32”）全面采用专用图层驱动与等宽数字字号，彻底消除系统 Cell 边距不对称与字形偏心问题，实现亚像素级完美水平与垂直对称居中。
- **完备的标注工具阵列**：
...
### 8. 偏好设置、关于与一键更新检测 *(v1.1.0 新增)*
- **独立「关于」面板**：展示原生高清 App 图标、当前版本号与构建号、项目特性亮点及 MIT 开源许可证。
- **免 API 限流的智能版本检测**：基于轻量 HEAD 重定向探测，无需 GitHub Token、绝无 403 限流风险，一键秒级检测最新版本；菜单栏同步提供「检查更新...」快捷入口。
- **开源社区传送门**：一键直达 GitHub 仓库与 Issue 反馈提交页面。
- **系统设置与全局热键**：严格遵循 macOS 设计规范，采用 160pt 原生侧边栏与 SF Pro 系统字体；全屏游戏或 IDE 重度编码场景下依然毫秒响应。
...
| 快捷键 | 功能操作 |
| :--- | :--- |
| `F1` / `Option + A` | 唤起标准屏幕截图 |
| `Option + F1` / `Option + S` | 唤起长截图模式 *(v1.1.0 新增)* |
| `Shift + F1` / `F2` / `Option + O` | 选区 OCR 识别与原地屏幕翻译 |
...
1. 前往 [GitHub Releases](https://github.com/iskf/SnipSnap/releases/latest) 下载最新的 `SnipSnap-1.1.0.dmg`。
